#pragma once

#include "common.hpp"
#include "render_pass.hpp"
#include "../nodes/projectable.hpp"

#include <algorithm>
#include <functional>
#include <limits>
#include <memory>
#include <stdexcept>
#include <string>
#include <utility>
#include <vector>

namespace nicxlive::core {

using RenderCommandBuilder = std::function<void(RenderCommandEmitter&)>;

namespace detail {
struct RenderItem {
    float zSort{0};
    std::size_t sequence{0};
    RenderCommandBuilder builder{};
};

struct RenderPass {
    RenderPassKind kind{RenderPassKind::Root};
    std::size_t token{0};
    float scopeZSort{0};
    std::weak_ptr<nodes::Projectable> projectable{};
    DynamicCompositePass dynamicPass{};
    std::vector<RenderItem> items{};
    std::size_t nextSequence{0};
    RenderCommandBuilder dynamicPostCommands{};
};
} // namespace detail

// Layered render graph builder (D: render.graph_builder.RenderGraphBuilder)
class RenderGraphBuilder {
public:
    RenderGraphBuilder() { ensureRootPass(); }

    void beginFrame() {
        nodes::advanceProjectableFrame();
        clear();
    }

    void clear() {
        passStack_.clear();
        nextToken_ = 0;
        ensureRootPass();
    }

    bool empty() const {
        if (passStack_.empty()) return true;
        if (passStack_.size() == 1) {
            return passStack_.front().items.empty();
        }
        return false;
    }

    void enqueueItem(float zSort, RenderCommandBuilder builder) {
        enqueueItem(zSort, RenderScopeHint::root(), std::move(builder));
    }

    void enqueueItem(float zSort, const RenderScopeHint& hint, RenderCommandBuilder builder) {
        if (!builder || hint.skip) return;
        auto& pass = resolvePass(hint);
        addItemToPass(pass, zSort, std::move(builder));
    }

    std::size_t pushDynamicComposite(const std::shared_ptr<nodes::Projectable>& composite,
                                     const DynamicCompositePass& passData,
                                     float zSort) {
        ensureRootPass();
        detail::RenderPass pass;
        pass.kind = RenderPassKind::DynamicComposite;
        pass.projectable = composite;
        pass.dynamicPass = passData;
        pass.scopeZSort = zSort;
        pass.token = ++nextToken_;
        pass.nextSequence = 0;
        pass.dynamicPostCommands = nullptr;
        passStack_.push_back(pass);
        if (auto c = composite) {
            c->dynamicScopeActive = true;
            c->dynamicScopeToken = pass.token;
        }
        return pass.token;
    }

    void popDynamicComposite(std::size_t token, RenderCommandBuilder postCommands = nullptr) {
        if (passStack_.size() <= 1) throw std::runtime_error("RenderGraphBuilder.popDynamicComposite called without matching push. " + stackDebugString());
        auto targetIndex = findPassIndex(token, RenderPassKind::DynamicComposite);
        if (targetIndex == 0) throw std::runtime_error("RenderGraphBuilder.popDynamicComposite scope mismatch (token=" + std::to_string(token) + ") " + stackDebugString());
        if (postCommands) {
            passStack_[targetIndex].dynamicPostCommands = postCommands;
        }
        while (passStack_.size() - 1 > targetIndex) {
            finalizeTopPass(true);
        }
        auto& pass = passStack_.back();
        if (pass.token != token) throw std::runtime_error("RenderGraphBuilder.popDynamicComposite scope mismatch (token=" + std::to_string(token) + ") " + stackDebugString());
        finalizeDynamicCompositePass(false, postCommands);
    }

    void playback(RenderCommandEmitter* emitter) {
        if (!emitter) return;
        if (passStack_.size() != 1) throw std::runtime_error("RenderGraphBuilder scopes were not balanced before playback. " + stackDebugString());
        auto rootItems = collectPassItems(passStack_.front());
        clear();
        playbackItems(rootItems, *emitter);
    }

private:
    using RenderItem = detail::RenderItem;
    using RenderPass = detail::RenderPass;

    std::vector<RenderPass> passStack_{};
    std::size_t nextToken_{0};

    void ensureRootPass() {
        if (!passStack_.empty()) return;
        RenderPass root;
        root.kind = RenderPassKind::Root;
        root.token = 0;
        root.scopeZSort = 0;
        root.nextSequence = 0;
        passStack_.push_back(root);
    }

    RenderPass& currentPass() {
        ensureRootPass();
        return passStack_.back();
    }

    static bool itemLess(const RenderItem& a, const RenderItem& b) {
        if (a.zSort == b.zSort) return a.sequence < b.sequence;
        return a.zSort > b.zSort;
    }

    std::vector<RenderItem> collectPassItems(RenderPass& pass) {
        if (pass.items.empty()) return {};
        std::sort(pass.items.begin(), pass.items.end(), itemLess);
        return pass.items;
    }

    static void playbackItems(const std::vector<RenderItem>& items, RenderCommandEmitter& emitter) {
        for (const auto& item : items) {
            if (item.builder) item.builder(emitter);
        }
    }

    void addItemToPass(RenderPass& pass, float zSort, RenderCommandBuilder builder) {
        if (!builder) return;
        RenderItem item;
        item.zSort = zSort;
        item.sequence = pass.nextSequence++;
        item.builder = std::move(builder);
        pass.items.push_back(std::move(item));
    }

    std::size_t findPassIndex(std::size_t token, RenderPassKind kind) const {
        if (passStack_.size() <= 1) return 0;
        for (std::size_t idx = passStack_.size(); idx > 0; --idx) {
            const auto& pass = passStack_[idx - 1];
            if (pass.token == token && pass.kind == kind) {
                return idx - 1;
            }
        }
        return 0;
    }

    std::size_t parentPassIndexForDynamic(const std::shared_ptr<nodes::Projectable>& composite) const {
        if (!composite || passStack_.empty()) return 0;
        auto ancestor = composite->parentPtr();
        while (ancestor) {
            if (auto proj = std::dynamic_pointer_cast<nodes::Projectable>(ancestor)) {
                auto token = proj->dynamicScopeTokenValue();
                auto idx = findPassIndex(token, RenderPassKind::DynamicComposite);
                if (idx > 0) return idx;
            }
            ancestor = ancestor->parentPtr();
        }
        return 0;
    }

    void finalizeDynamicCompositePass(bool autoClose, RenderCommandBuilder postCommands = nullptr) {
        if (passStack_.size() <= 1) throw std::runtime_error("RenderGraph: cannot finalize dynamic composite scope without active pass. " + stackDebugString());
        auto pass = passStack_.back();
        passStack_.pop_back();

        auto childItems = collectPassItems(pass);
        auto composite = pass.projectable.lock();
        auto finalizer = postCommands ? postCommands : pass.dynamicPostCommands;

        RenderCommandBuilder builder = [composite, passData = pass.dynamicPass, childItems, finalizer](RenderCommandEmitter& emitter) {
            emitter.beginDynamicComposite(composite, passData);
            playbackItems(childItems, emitter);
            emitter.endDynamicComposite(composite, passData);
            if (finalizer) finalizer(emitter);
        };
        auto parentIndex = parentPassIndexForDynamic(composite);
        addItemToPass(passStack_[parentIndex], pass.scopeZSort, std::move(builder));

        if (autoClose && composite) {
            composite->dynamicScopeActive = false;
            composite->dynamicScopeToken = std::numeric_limits<std::size_t>::max();
        }
    }

    void finalizeTopPass(bool autoClose) {
        if (passStack_.size() <= 1) throw std::runtime_error("RenderGraph: no scope available to finalize. " + stackDebugString());
        auto kind = passStack_.back().kind;
        switch (kind) {
        case RenderPassKind::DynamicComposite:
            finalizeDynamicCompositePass(autoClose, nullptr);
            break;
        case RenderPassKind::Root:
            throw std::runtime_error("RenderGraph: cannot finalize root pass.");
        }
    }

    RenderPass& resolvePass(const RenderScopeHint& hint) {
        ensureRootPass();
        std::size_t index = 0;
        switch (hint.kind) {
        case RenderPassKind::Root:
            index = 0;
            break;
        case RenderPassKind::DynamicComposite:
            index = findPassIndex(hint.token, RenderPassKind::DynamicComposite);
            if (index == 0) throw std::runtime_error("RenderGraph: dynamic composite scope not active for enqueue. " + stackDebugString());
            break;
        }
        return passStack_[index];
    }

    std::string stackDebugString() const {
        std::string buf = "[stack depth=" + std::to_string(passStack_.size()) + "]";
        for (std::size_t i = 0; i < passStack_.size(); ++i) {
            buf += " #" + std::to_string(i) + "(kind=" + std::to_string(static_cast<int>(passStack_[i].kind)) +
                   ",token=" + std::to_string(passStack_[i].token) + ")";
        }
        return buf;
    }
};

} // namespace nicxlive::core
