...
| [>] | CP148 | behavior_diff | `compat-part.md`, `compat-command_emitter.md`, `compat-grid_deformer.md` | `ApplyMask` が `BeginMask` なしで出るコマンド列不整合（`Midori-gridtest2`） | `GridDeformer::deserializeFromFghj` の `formation` 読み取りが `int` 固定で例外化し、`Body` 系 subtree のデシリアライズが途中停止していた。D写経で文字列/数値両対応へ修正後、`/tmp/run-midori-gridtest2-nicx-current.txt` で失敗ログ消失、`emit out count=201` まで回復。`/tmp/queue-midori-gridtest2-nicx-current.log` では `BeginMask -> ApplyMask* -> BeginMaskContent -> EndMask` 順序が成立。 | 残差分（`nijilive 244` vs `nicxlive 201`、`BeginMask 17` vs `7`）を継続追跡する。次は `Part/Composite` の `masks` 復元順と `masked_by` 系のD挙動を関数単位で再突合し、欠落している mask block を特定する。 |
