...
| [>] | CP141 | behavior_diff | `compat-path_deformer.md` | Midori の `Upper Leg::*::PD` / `Lower Leg::*` で `d0` が過大（袖非表示の主因候補） | `PathDeformer` の `setupChildNoRecurse/releaseChildNoRecurse`、`captureTarget(prepend)`、`cacheClosestPoints` を D 写経へ寄せたが、`queue-midori-nicx-f1-20260223m.log` で `do=7515/7546/7921/7951` は依然 `|d0|≈10~14`（`nijilive` は `|d0|≈1~1.5`）で未解消。 | 次は `deformer/path.d` の `deformChildren` と `drivers/phys.d` の入力更新順（`prevCurve`/`inverseMatrix`/driver update）を関数単位で再写経し、`queue-midori-*-f1` の該当 `do` を一致させる。 |
