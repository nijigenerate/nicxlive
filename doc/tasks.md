...
| [>] | CP133 | behavior_diff | `compat-composite.md`, `compat-projectable.md` | Puppet scale 変化時の `Composite` 再初期化/再描画フロー | `Projectable::rebuffer` と `Composite::rebuffer` の写経欠落を修正し、`rebuffer -> initialized=false -> notifyChange` と `autoResizedMesh=true` 維持を追加。`NJIV_AUTO_WHEEL_INTERVAL=1, PHASE_TICKS=100, --frames 220` で再検証した結果、`vo=916/1563/1504` は frame 0-123 で nicx が過小（例 frame0: `-43` vs `-373`）だが frame 124 以降（scale>1）でほぼ一致（差分 ~1-3）へ収束。加えて同ログで frame124 以降に `initTarget type=Composite` と `create h=... rt=true` が連続発火しており、拡大局面で `initTarget -> createTexture` 連鎖は実行されていることを確認 | `nijilive` の `composite.d` / `projectable.d` と C++ 実装を関数単位で再比較し、Dに存在する契約のみを実装する |
| [>] | CP135 | behavior_diff | `compat-drawable.md`, `compat-projectable.md` | `Drawable::updateBounds` の座標系不一致で `initTarget` がスケール非反映サイズを使う | D版は `getDynamicMatrix` でワールド bounds を計算するが、C++版はローカルAABBのみで更新していたため、`Projectable::initTarget` が固定に近い `tex=(91,81)/(26,19)/(26,17)` を作成し続ける差分が発生。`core/nodes/drawable.cpp` をD写経で修正し、world bounds 計算へ戻した（検証ログ再取得は次ステップ）。 | `nijilive/core/nodes/drawable.d:updateBounds` を正として、`nicxlive/core/nodes/drawable.cpp:updateBounds` を同等ロジックへ写経。`njiv --test` の大スケール操作で `initTarget tex=(...)` がスケール追従することを確認する。 |
